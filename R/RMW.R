...
#' 
#' Apply Robust Multivariate Winsorization to a dataset.
#' 
...
#' uses robust Mahalanobis distance multivariate outlier identification (\code{\link{RobMulOutliers}}). Given that the 
...
#' \strong{multivariate outlier cases will be shifted directly toward the data's centroid.}
#' 
...
#' cutoff of 3 standard deviations (i.e., about `r round(pnorm(-3)*2,4)`).
#' 
#' Note that this method's influence on multivariate Gaussian distributions' bivariate product-moment
#' correlations has been informally shown to be minimal and unbiased (e.g., in testing of data
...
#' 
#' This is the final step in the full \code{\link{MVNclean}} data cleaning pipeline.
#'
#' @seealso  
#' 
#' Dixon and Tukey (\code{doi.org/10.2307/1266226}) ; 
#' 
...
#'    \item{\code{shiftedCases}}{Logical vector indicating whether each index is was identified as an outlier.}
...
#'    \item{\code{origDat}}{\emph{optional} original \code{dat}.}
#' }
#'
#' @export
#'
...
RMW <- function(dat
  ...
  dat <- check_MVN_compatibility(dat)
  if(any(dim(dat$dat_nonNum))>0){
    isNonNumeric <- T
    dat_nonNum <- dat$dat_nonNum
  }else{
    isNonNumeric <- F
  }
  dat <- dat$dat
  ...
  if(nrow(dat) < 10){
    stop(paste('The data too few observations.'))
  }    
  if(nrow(dat) < 25){
    warning(paste('Interpret results with caution; the data has very few observations.'))
  }
